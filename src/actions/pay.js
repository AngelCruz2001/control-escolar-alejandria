...
import {
  ...
  uiFinishLoadingStudents,
  uiStartLoading,
  uiStartLoadingCards,
  uiStartLoadingStudents,
} from "./ui";
  ...
  return async (dispatch, getState) => {
    try {
      ...
      const res = await fetchConToken(
        `payments/students/${getState().pay.active.matricula}/check`,
        dataToSend,
        "POST"
      );
      const body = await res.json();
...
export const payStartGetAllPayments = () => {
  return async (dispatch, getState) => {
    try {
      ...
      const body = await res.json();
      // dispatch(paySetActivePay(body.payments));
      if (body.ok) {
...
export const payStartGetFertilizerPay = (matricula) => {
  return async (dispatch) => {
    try {
      ...
      if (body.ok) {
        console.log(body);
        dispatch(paySetFertilizers(body.student.payments, matricula));
      } else {
        console.log(body);
        Swal.fire({
          title: "¡Oops!",
...
export const payStartGetStudentsByGroup = (id_group, active) => {
  return async (dispatch) => {
    active && dispatch(paySetActivePay(active));
    try {
